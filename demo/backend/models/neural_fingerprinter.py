...
class SC(nn.Module):
    def __init__(
        ...
        out_H = int(
            np.floor(
                (H + 2 * padding_sizes[1][0] - kernel_sizes[1][0]) / stride_sizes[1][0]
                + 1
            )
        )
        out_W = int(
            np.floor(
                (W + 2 * padding_sizes[0][1] - kernel_sizes[0][1]) / stride_sizes[0][1]
                + 1
            )
        )
        ...
        self.separable_conv2d = nn.Sequential(
            self.separable_conv2d_1x3,
            nn.ReLU(),
            self.BN_1x3,
            self.separable_conv2d_3x1,
            nn.ReLU(),
            self.BN_3x1,
        )
